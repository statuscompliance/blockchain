...
function transformFunctionStatements(statement: Statement): string {
  ...
  if (statement.isKind(SyntaxKind.ExpressionStatement)) {
    ...
    if (expression) {
      ...
      const args = expression.getArguments()[0]?.getText();
 ...
 */
export function transformFunction(func: FunctionDeclaration, target: IBaseChaincodeAST): void {
  for(const statement of func.getStatements()){
